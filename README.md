...
### 工作区与性能
- **工作区搜索与安全替换** — 支持多文件全文搜索、正则、大小写、类型/忽略目录筛选、搜索历史和结果定位；跨文件替换始终先展示中文语义 Diff，再统一确认写入。
- **资源管理器升级** — 支持更多可转换文件，点击即可转换为 Markdown；文件类型图标升级为 VS Code 风格。
- **长文档优化** — Mermaid 图表延迟渲染、预览更新延后处理，并优化拖动分栏和工作区加载的性能路径。

### Markdown 与导出
- **公式与图表** — 完善 KaTeX 数学公式和 Mermaid 图表支持，内置流程图、时序图、甘特图快捷插入。
- **统一模板导出** — HTML、PDF、PNG 共用模板系统，支持文章、论文、公文、技术文档、演示长图、小红书长图、A4、README 与自定义 CSS。
- **公众号排版** — 新增六套原创主题的微信公众号富文本导出，可直接复制粘贴到公众号编辑器或保存 HTML。

### 可控 AI 与研究写作
- **AI Diff 确认** — 重写、翻译、摘要、大纲、伴写和校对默认进入建议模式，逐处确认、显示修改依据，并支持整轮撤销。
- **工作区上下文** — 支持文件/章节选择、本地片段检索、Token 估算、敏感文件排除和发送内容披露。
- **带来源的网络搜索** — 搜索结果保留来源信息与访问时间，可导出 Markdown 脚注。
