...
- 合并安装/更新按钮，在线安装前先预热连接检查是否有新版本可用。
- 修复无签名模式下 `fetch_manifest` 编译错误——`signature` 变量未定义。
...
- CI 运行环境 Node.js 从 20 升级到 24，`actions/checkout` 和 `actions/setup-node` 升级到 v5。
