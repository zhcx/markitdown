...
use super::model::{
    ...
    WebDavBackupRequest, WebDavDocumentRef, WebDavQueuedResult, WebDavRetryResult, WebDavSettings,
    WebDavSyncEvent, WebDavVersion,
};
...
/// Serialized background worker for WebDAV document backups.
///
/// Owns the durable pending-task store and a global worker mutex so manifest,
/// index, and current-copy updates never race across documents.
#[derive(Clone)]
...
impl WebDavSyncManager {
    ...
    ///
    /// Returns only after the task is durably queued; the network transaction
    ...
    pub async fn enqueue(
        ...
        self.inner.queue.upsert(task.clone())?;

        self.emit(WebDavSyncEvent {
        ...
        tauri::async_runtime::spawn(async move {
            let _ = manager.process(task, settings).await;
        });
    ...
    pub async fn retry_pending(
        ...
        let tasks = self.inner.queue.load()?;
        let count = tasks.len();
        for task in tasks {
            let manager = self.clone();
    ...
    pub async fn process(
        ...
        let _guard = self.inner.worker.lock().await;
        ...
        let result = self.sync_task(&mut task, &settings).await;
        match result {
            Ok(()) => {
                if let Err(error) = self.inner.queue.remove(&task.document_id) {
                    self.emit(WebDavSyncEvent {
                        document_id: task.document_id.clone(),
                        local_path: task.local_path.clone(),
    ...
    async fn sync_task(
        ...
        let mut manifest = match &manifest_bytes {
            ...
            None => {
                DocumentManifest::new(&task.document_id, &task.display_name, &task.current_path)
            }
        };
    ...
    fn emit(&self, event: WebDavSyncEvent) {
