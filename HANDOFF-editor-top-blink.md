...
### 防护 3：字形尺寸清零与视口裁剪（`main.css`）
针对输入承载层（`.inputarea`, `.ime-input`, `.ime-text-area`, `.native-edit-context`）：
- 强制添加 `font-size: 0px !important; line-height: 0px !important; letter-spacing: -9999px !important; clip: rect(0, 0, 0, 0) !important;`。
- 即使 WebView2/TSF 原生试图在 textarea 内部渲染字符，字形尺寸为 0 且被绝对裁剪，无法渲染出任何像素。
...
1. **自动化测试**：
   - 运行 `npm test`：**139/139 全部通过**（包含专门新增的顶部遮罩、输入层尺寸清零与动态钳位逻辑测试）。
   - 运行 `npm run lint`：**0 错误，0 警告**。
2. **Git 提交**：
   - 提交哈希：`c47f9ad`
   - 分支：`fix/editor-top-blink-v0.4.4`（已推送到 `origin`）
3. **最新全量打包产物**（构建时间：2026-09-03 16:31）：
   - **NSIS 安装包**（推荐）：
